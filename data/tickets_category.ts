...
class TicketsCategory {
    ...
    async init(){
        const res = await this.db.query(`SELECT * FROM TICKETS_CATEGORY WHERE id = ? OR name = ?`, [`${this.idORname}`, `${this.idORname}`])
    ...
    async updateName(name: string){
        if (!this.id) throw new Error("Email not found");
        this.db.query(`UPDATE TICKETS_CATEGORY SET name = ? WHERE id = ?`, [`${name}`, this.id]);
    ...
    async updateRegularPrice(regularPrice: number){
        if (!this.id) throw new Error("Email not found");
        await this.db.query(`UPDATE TICKETS_CATEGORY SET regular_price = ? WHERE id = ?`, [`${regularPrice}`, this.id]);
        this.idORname = this.id
        await this.init();
    }
    ...
    async updateChildrenPrice(childrenPrice: number){
        if (!this.id) throw new Error("Email not found");
        await this.db.query(`UPDATE TICKETS_CATEGORY SET children_price = ? WHERE id = ?`, [`${childrenPrice}`, this.id]);
        this.idORname = this.id
        await this.init();
    }
    ...
    async updateStudentPrice(studentPrice: number){
        if (!this.id) throw new Error("Email not found");
        await this.db.query(`UPDATE TICKETS_CATEGORY SET student_price = ? WHERE id = ?`, [`${studentPrice}`, this.id]);
        this.idORname = this.id
        await this.init();
    }
    ...
    async updateAudioguidePrice(audioguidePrice: number){
        if (!this.id) throw new Error("Email not found");
        await this.db.query(`UPDATE TICKETS_CATEGORY SET audioguide_price = ? WHERE id = ?`, [`${audioguidePrice}`, this.id]);
        this.idORname = this.id
        await this.init();
    }
    ...
    async updateCanAccAMEA(canAccAMEA: boolean){
        if (!this.id) throw new Error("Email not found");
        await this.db.query(`UPDATE TICKETS_CATEGORY SET can_acc_AMEA = ? WHERE id = ?`, [`${canAccAMEA}`, this.id]);
        this.idORname = this.id
        await this.init();
    }
